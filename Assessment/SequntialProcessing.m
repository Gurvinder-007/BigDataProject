function [RunTimeSeq]=SequntialProcessing(LoopParameter,FileName)

%% This script allows you to open and explore the data in a *.nc file

close all

%%FileName = 'C:/Users/Gurvinder Nagra/Documents/GitHub/5011CEM2022NagraG4/Model/o3_surface_20180701000000.nc';

Contents = ncinfo(FileName);

TestNanErrors(FileName);

Lat = ncread(FileName, 'lat'); % load the latitude locations
Lon = ncread(FileName, 'lon'); % loadthe longitude locations

Segments= 1000;
IncrementsProcessed = 1;
...
tic
for NumHour = 1: IncrementsProcessed % loop through each hour
    ...
    t1 = toc;
    t2 = t1;
    for idx = LoopParameter:  Segments %size(Data2Process,1) % step through each data location to process the data
